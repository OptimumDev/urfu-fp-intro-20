...
map f xs = foldr (\x b -> f x :. b) Nil xs
...
filter f xs = foldr (\x b -> if f x then x :. b else b) Nil xs
...
-}
toListH :: List a -> [a]
toListH xs = foldr (\x b -> x : b) [] xs

-- И обратно
fromListH :: [a] -> List a
fromListH xs = P.foldr (\x b -> x :. b) Nil xs
...
-}
intercalate :: [a] -> [[a]] -> [a]
intercalate sep (l : ls) = l ++ P.foldl(\b x -> b ++ sep ++ x) [] ls
